...
class HeightScanner
{
    ...
    public: 
        ...
        */
         ...
         */
        void footScan(double base_yaw);

        /**
         * @brief Calculate the foot clearance reward given the feet position.
         */
        double footClearanceReward(const Eigen::Vector4d &foot_phases);
         ...
         *      will be added
         */
        void addVisualIndicators(raisim::RaisimServer* server);
};
...
void HeightScanner::footScan(double base_yaw)
{
    ...
    for (int i = 0; i < this->n_legs_; i++)
    {
        ...
        for (int j = 0; j < (this->scans_per_foot_); j++)
        {
            ...
            this->feet_height_scan[i * (this->scans_per_foot_) + j] = (
                col[0].getPosition()[2] - foot_position_eigen[2]
            );
...
double HeightScanner::footClearanceReward(const Eigen::Vector4d &foot_phases){
...
void HeightScanner::addVisualIndicators(raisim::RaisimServer* server)
{
